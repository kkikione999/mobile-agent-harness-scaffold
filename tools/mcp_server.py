...
def _snapshot_introspection(snapshot: dict[str, Any]) -> dict[str, Any]:
    ...
    for element in elements:
        ...
        if bool(element.get("visible", True)):
            visible_elements += 1
        if bool(element.get("interactive")):
    ...
    return {
        ...
        "capture_error": capture_error if isinstance(capture_error, dict) else None,
    }


def _semantic_full_snapshot_request() -> dict[str, bool]:
    return {"interactive_only": False, "compact": False, "bridge_first_full": True}


def _semantic_full_snapshot(session: DeviceSession) -> dict[str, Any]:
    return session.driver.snapshot(_semantic_full_snapshot_request())


def _semantic_snapshot_ready(snapshot_summary: dict[str, Any]) -> bool:
    screen_id = snapshot_summary.get("screen_id")
    semantic_id_count = snapshot_summary.get("semantic_id_count")
    return (
        snapshot_summary.get("capture_source") == "android_accessibility_bridge"
        and isinstance(screen_id, str)
        and bool(screen_id)
        and isinstance(semantic_id_count, int)
        and semantic_id_count > 0
    )


def _semantic_settlement_supported(snapshot_summary: dict[str, Any]) -> bool:
    capture_source = snapshot_summary.get("capture_source")
    return (
        isinstance(capture_source, str)
        and capture_source != "unknown"
    ) or isinstance(snapshot_summary.get("capture_error"), dict)


def _settle_open_semantics(session: DeviceSession) -> dict[str, Any]:
    deadline = time.monotonic() + (SETTLEMENT_TIMEOUT_MS / 1000.0)
    attempts = 0
    stable_observations = 0
    last_tree_hash: Any = None
    last_summary = _snapshot_introspection({})

    while True:
        snapshot = _semantic_full_snapshot(session)
        attempts += 1
        last_summary = _snapshot_introspection(snapshot)

        if not _semantic_settlement_supported(last_summary):
            return {
                "status": "unsupported",
                "attempts": attempts,
        ...
        if _semantic_snapshot_ready(last_summary):
            ...
            if stable_observations >= SETTLEMENT_STABLE_OBSERVATIONS:
                settled_snapshot, _ = _cached_snapshot(session, interactive=True, compact=True, refresh=True)
                return {
                    "status": "ready",
                    "attempts": attempts,
                    "stable_observations": stable_observations,
                    "snapshot": _snapshot_introspection(settled_snapshot),
                }
        else:
            stable_observations = 0
            last_tree_hash = None
        ...
        time.sleep(SETTLEMENT_POLL_MS / 1000.0)
...
def _build_page_map(snapshot: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(elements, list) or not elements:
        return {
            ...
            "page": {"root": None, "signature": {}, "sections": [], "interactive_refs": []},
        }
...
def _tool_device_open(arguments: dict[str, Any], runner: Runner) -> tuple[bool, dict[str, Any]]:
    ...
    settlement = None
    if platform == "android" and dispatch_commands and status != "error":
        settlement = _settle_open_semantics(session)

    result = {"launch": launch, "preflight": preflight}
    if isinstance(settlement, dict):
        result["settlement"] = settlement
    result_json = {"status": status, "operation": "open", "result": result}
...
def _tool_device_press(arguments: dict[str, Any], runner: Runner) -> tuple[bool, dict[str, Any]]:
    ...
    if result.get("status") in {"error", "fail"} and result.get("error_code") == "selector_drift":
        full_snapshot = _semantic_full_snapshot(session)
        retry_context = _selector_retry_context(selector, full_snapshot) if isinstance(full_snapshot, dict) else None
        full_elements = full_snapshot.get("elements", []) if isinstance(full_snapshot, dict) else []
        if not isinstance(full_elements, list):
            full_elements = []
        result = session.driver.interact({"action": "tap", "selector": selector}, elements=full_elements)
